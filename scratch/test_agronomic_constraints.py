...
test_windows = [
    ...
    ("Urban Settlement (r=300, c=1520)", rasterio.windows.Window(1520, 300, 256, 256)),
...
for name, win in test_windows:
    ...
    # 农学物理硬约束：
    # 1. 峰值 NDVI 必须达到 0.35 (排除建筑/水体/道路/裸土)
    # 2. 农田必须有翻耕/休耕阶段 (ndvi_min <= 0.32 或 ndvi_range >= 0.28，常绿森林 ndvi_min > 0.35 且始终保持高位)
    nd_arr = np.stack(raw_ndvis, axis=-1)
    nd_max = np.max(nd_arr, axis=-1)
    nd_min = np.min(nd_arr, axis=-1)
    nd_range = nd_max - nd_min
    
    pure_crop = (mask == 1) & (nd_max >= 0.35) & ((nd_min <= 0.32) | (nd_range >= 0.28))
    
    print(f"Region: {name:32s}")
    print(f"  Raw Model Cropland: {np.mean(mask == 1)*100:5.2f}%")
    print(f"  Agronomic Physical Cropland: {np.mean(pure_crop)*100:5.2f}%")
